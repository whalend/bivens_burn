...
Best model formulation based on AIC uses maximum temperature variable 
...
summary(m25cm1)
rsquared(m25cm1)
MuMIn::r.squaredGLMM(m25cm1)
plot_model(m25cm1, show.values = T)
plot_model(m25cm1, type = "pred", pred.type = "re", show.data = T)

m25cm2 <- glmer(
  survival ~ ht2_cm + avg_flameht_cm + avg_max_temp + (1|Plot), 
  family = binomial(link = logit), data = dat25cm)
plot_model(m25cm2, type = "pred", pred.type = "re", show.data = T)
...
plot_model(tree_ht_mod, show.values = T)
summary(tree_ht_mod)

```


```{r plot level response models}
tree_plots_dat <- filter(agg_plot_data, Plot %in% unique(dat25cm$Plot))

## flame height model ####
flame_ht_mod <- lm(avg_flameht_cm ~ avg_drywt + veg_trt, data = filter(tree_plots_dat, probe_ht == "25cm"))
# plot_model(flame_ht_mod, type = "diag")
plot_model(flame_ht_mod, type = "pred", terms = c("avg_drywt","veg_trt"), show.data = T)
summary(flame_ht_mod)

flame_ht_fig <- ggplot(filter(tree_plots_dat, probe_ht == "25cm"),
                       aes((avg_drywt*16)/1000, avg_flameht_cm),
                       ) +
  geom_point(aes(color = veg_trt, shape = water_trt), alpha = 1) +
  scale_shape_discrete(solid = FALSE) +
  invasion_color +
