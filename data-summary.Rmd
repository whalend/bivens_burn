---
title: "Bivens Burn Data Summary and Exploration"
author: "W.W. Dillon"
date: "`r Sys.Date()`"
output:
  html_document:
...
temps_abv_100_data <- temperatures %>% 
  group_by(Plot, Location, probe_ht, water_trt, veg_trt) %>% 
...
agg_fuel_flame_data <- fuel_flame_data %>% 
  ...
  summarise(avg_dry_wt = mean(drywt),
...
```{r fuel height boxplot}

ggplot(filter(agg_plot_data, probe_ht=="0cm"), aes(water_trt, avg_fuel_ht_cm)) +
  geom_boxplot(aes(color = veg_trt), notch = F, outlier.color = "white") +
  geom_point(aes(color = veg_trt), position = position_jitterdodge(dodge.width = .8, jitter.width = .2)) +
  # stat_summary(aes(color = veg_trt),fun.data = "mean_se", geom = "pointrange", position = position_dodge(width = .2), size = 1) +
  invasion_fill +
...
knitr::kable(filter(agg_plot_data, probe_ht=="0cm") %>% 
    group_by(water_trt, veg_trt) %>% 
    summarise(avg_fuel_ht = mean(avg_fuel_ht_cm),
              nobs = length(avg_fuel_ht_cm),
              se_fuel_ht = sd(avg_fuel_ht_cm)/sqrt(nobs)), 
    digits = 2, caption = "Fuel height (cm)")
```
...
Two observations of flame height at three locations in each plot, with ten plots in each treatment group. Average flame heights estimated to be 4 time higher in cogongrass-invaded plots compared to reference plots, regardless of drought.
...
fig_labels <- data.frame(
  ...
  panel_id = c("a","b","c"),
  avg_dry_wt=c(-.1,-.1,-.1), 
  avg_s_abv_100=c(450,450,450))

time_abv_100_fig <- ggplot(agg_plot_data, aes((avg_dry_wt*16)/1000, avg_s_abv_100)) +
...
```{r temps abv 100 table, eval=T}

knitr::kable(temps_abv_100_data %>% 
         group_by(probe_ht, veg_trt, water_trt) %>% 
         summarise(avg_s_abv100 = mean(seconds_abv_100),
                   n_tc_obs = length(seconds_abv_100),
                   se_s_abv100 = sd(seconds_abv_100/sqrt(n_tc_obs))),
...
maxt_mod <- (lm(avg_max_temp ~ avg_dry_wt + veg_trt + probe_ht, data = agg_plot_data))
summary(maxt_mod)
car::Anova(maxt_mod)

sjPlot::plot_model(maxt_mod, type = "pred", terms = c("avg_dry_wt","veg_trt","probe_ht"), show.data = T) +
  invasion_color +
  invasion_fill
...
```{r max temp summary tables, eval=FALSE}
...
max_temps50_lm <- lm(avg_max_temp ~ avg_dry_wt + veg_trt, data = filter(agg_plot_data, probe_ht=="50cm"))
summary(max_temps50_lm)

equatiomatic::extract_eq(max_temps50_lm, use_coefs = TRUE) %>%
  equatiomatic::preview()

ggplot(filter(agg_plot_data, probe_ht=="50cm"), aes(.001*avg_dry_wt*16, avg_max_temp)) +
  geom_smooth(aes(fill = veg_trt, color = veg_trt), alpha = .3, method = "lm", show.legend = FALSE) +
  geom_point(aes(color = veg_trt, shape = water_trt)) +
  scale_shape_discrete(solid = FALSE) +
  invasion_color +
  invasion_fill +
...
max_temps25_lm <- lm(avg_max_temp ~ avg_dry_wt + veg_trt, data = filter(agg_plot_data, probe_ht=="25cm"))
summary(max_temps25_lm)
...
max_temps0_lm <- lm(avg_max_temp ~ avg_dry_wt + veg_trt, data = filter(agg_plot_data, probe_ht=="0cm"))
summary(max_temps0_lm)
...
```{r test probe ht models, eval=F}
library(lme4)

m0cm <- glmer(
  survival ~ tree_ht_scl + # tree ht alone, AIC: 86.9
    flame_ht_scl + # only this + tree ht, AIC: 64.9
...
## Tree height alone, AIC: 86.9
## Tree height + flame height, AIC: 65
## No effective change in AIC by including a temperature variable
...
# library(rstanarm)
# m50cm_stan <- stan_glmer(survival ~ tree_ht_scl + flame_ht_scl + hflux_scale + (1|Plot), family = binomial(link="logit"), data = dat50cm)
# summary(m50cm_stan)
# # median(bayes_R2(surv1))
...
The best model based on AIC uses maximum temperature at 25 cm when compared to other models with alternate temperature variables (probe height or temperature residence time/heat flux >100 ºC), but adding a temperature variable doesn't improve AIC over a model with only tree height and flame height.
