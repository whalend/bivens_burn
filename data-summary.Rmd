...
# invasion_color <- scale_color_manual(values = c("green","tan4"))
...
## Max temp regressions: 50cm probe height
```{r max temp regressions}
...
max_temp50_lme4 <- lme4::lmer(max_temp ~ avg_drywt + veg_trt + avg_fuel_ht_cm + (1|Plot), data = max_temps50_data)
...
sjPlot::plot_model(
  ...
  vline.color = "gray90", colors = "viridis",
  title = "Standardized max temp (divided by 2*sd") +
  theme_bw()
