...
```{r fuels data summary table, eval=FALSE}
...
```{r fit maximum temperature models}
...
```{r fig1 C D and figS1 temperature figures}
...
```{r Fig1 treatment effects on fire, fig.width=9, fig.height=3}
...
```{r Fig2C global change prediction}
...
```{r Fig2 panels D and E and Fig S2 mechanistic model prediction}
...
```{r Fig2 A and B tree height vs flame height and mortality}
df2 <- dat25cm %>% 
...
```{r Figure 2 global drivers and mechanisms, fig.width=8, fig.height=8}
# glob_drivers_mod
# th_fh_mort_fig
# ht_mech_pred_fig
# fh_mech_pred_fig
...
```{r Figure 3 managment model, fig.height=7, fig.width=7}
...
```{r TableS1 fuels model estimates}
...
```{r TableS2 flame height model estimates}
...
```{r TableS3 maximum temperature models summaryies}
...
```{r TableS4 heating duration models}
...
```{r Table S5 treatment effects summary}
dat25cm %>% 
  group_by(Treatment) %>% 
...
```{r Table S6 global change model}
...
```{r Table S7 global change predictions}
...
```{r Table S8 mechanistic model summary}
...
```{r Table S9 mechanistic model predictions for specific tree and flame heights}
...
```{r Table S10 management model summary}
...
```{r Table S11 management model prediction}
surv_fuel_pred <- plot_model(
  management_model, type = "pred", 
...
```{r Figure S1 max temp heating duration, fig.height=8, fig.width=7}
...
```{r Figure S2 mech_model_supp_fig, fig.width=7, fig.height=3.5}
